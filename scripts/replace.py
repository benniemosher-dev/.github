...
infracost_command = Template('infracost breakdown --format=$format --no-color --path=$path')
...
def replace_chunk(content, marker, chunk, inline=False):
    ...
    if not inline:
        chunk = '\n{}\n'.format(chunk)
    chunk = '<!-- {}_starts -->\n```{}```\n<!-- {}_ends -->'.format(marker, chunk, marker)
    return r.sub(chunk, content)

if __name__ == "__main__":
    output_stream = os.popen(infracost_command.substitute(path=args.path, format=args.format))
